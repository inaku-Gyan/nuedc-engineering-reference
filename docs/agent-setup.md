...
生成位置始终以 `configure_agent.py` 自身所在目录为准，而不是命令执行时的当前
工作目录。因此也可以从其他目录通过脚本的完整或相对路径调用；配置和提示词仍会
写入知识库根目录。
...
创建完整资料包并更新索引。不得无界扩展相邻主题。如果资料因登录、验证码、地区
限制、动态页面或其他访问障碍而无法下载或查看，Agent 应向用户说明资料名称或
文档编号、版本、来源链接、所需文件以及受阻原因，请用户协助取得后继续整理。
